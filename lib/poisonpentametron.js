...
//FIXME: added to separate library
pp.censorWord = function (word) {
  var _s = require('underscore.string');
  return word.substring(0,1) + _s.repeat("*", (word.length-1));
};

pp.censorTweets = function (tweet) {
...
pp.downloadFeed = function(url, cb) {
...
function cleanText(rawPoetry, cb) {
  ...
  rawPoetry.forEach(function(line) {
    ...
    // console.log("stripChars: %s", stripChars);

    var censoredTweet = pp.censorTweets(stripChars);
    // console.log("censored: %s", censoredTweet);
...
pp.updateFeed = function(cb) {
  var waterfall = [start, pp.downloadFeed, parseTweets, saveRawFile, cleanText, createMatrix, createPlainText, debugCopyFiles];
