      ...
      fs.writeFile(config.feed, body, 'utf-8', function(err) {
        if (err) {
          cb(err);
            ...
            fs.writeFile(config.feed + "." + dayNo, body, 'utf-8', function(err) {
              if (err) {
                cb(err);
              } else {
                cb(null);
...
function saveRawFile(fileName, rawPoetry, encoding, cb) {
  ...
  fs.writeFile(fileName, JSON.stringify(rawPoetry, null, 4), encoding, function(err) {
    if (err) {
      ...
      fs.writeFile(fileName + "." + dayNo, JSON.stringify(rawPoetry, null, 4), encoding, function(err) {
        if (err) {
          cb(err);
        } else {
            cb(null, rawPoetry);
...
function cleanText(rawPoetry, cb) {
  ...
  fs.writeFile(config.cleanFile, JSON.stringify(cleanPoetry, null, 4), 'utf-8', function(err) {
    if (err) {
      ...
      fs.writeFile(config.cleanFile + "." + dayNo, JSON.stringify(cleanPoetry, null, 4), 'utf-8', function(err) {
        if (err) {
          console.error('\t\tcleanText error: %s', JSON.stringify(err, null, 4));
          cb(err);
        } else {
...
pp.updateFeed = function(cb) {
  var waterfall = [renameOldFeed, downloadFeed, parseTweets, saveRawFile, cleanText, createMatrix, createPlainText, debugCopyFiles];
